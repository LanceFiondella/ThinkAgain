(function (window){
	...
	p.playArea= new createjs.Container();
	p.alphaLocked = false;
	...
	p.getSavedGame = function(){
		...
		if (window.game.multiplayerGame)
			usr = "multi";
		else
	    ...
	    $.ajaxSetup({
	            ...
	            //if (!csrfSafeMethod(settings.type) && !this.crossDomain) {
	              ...
	              type: 'POST',
	              url: '/get_saved_game/',
	              data: "problem_name=" + sessionStorage.getItem("filename")+"&username="+ usr,
	              success: function(data){
	                    response =data;
	                },
	              dataType: "json",
	...
	p.addSavedPieces = function(){
		...
		if (result.steps){
			...
			if (this.trackTime < this.gameState.saved_steps[this.gameState.saved_steps.length-1].t) 
	...
	p.sendStep = function(step){
		...
		if (window.game.multiplayerGame)
			...
			usr = sessionStorage.getItem("username");
		dat = "problem_name=" + sessionStorage.getItem("filename")+"&username="+ usr+"&total_pieces=" + this.pm._total_pieces+ "&total_time=" + this.trackTime + "&solution=" + JSON.stringify(step)
	    var csrf_token = $.cookie('csrftoken');
