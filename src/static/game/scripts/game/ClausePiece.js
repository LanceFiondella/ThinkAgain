(function (window){
	...
	p.addMouseProperties = function(){
		this.on("mousedown",function(evt){
			console.log("UNsolved piece clicked scaleX " + this.scaleX);
			//Checks if the the piece has already been clicked scaleX and scaleY should be < 1.0
			if (this.scaleX == 1.0){
	...
	p.clausePieceShape = function(st_list, piece_num){
		//console.log("inside clause piece shape");
		this.keys = st_list;
		...
		if (keysLength == 0){
			...
			var star_image = new createjs.Bitmap("/static/game/images/star.jpg");
		...
		for (var i =0; i< keysLength; i++){
			...
			if (this.keys[i] > 0) {
					pieceLetterText = new createjs.Text((chr), "bold 85px Arial","black");
					atom.graphics.beginFill("white");
                	atom.graphics.beginStroke("black");
				}
			else{
					pieceLetterText = new createjs.Text((chr), "bold 85px Arial","white");
