...
function PieceManager(){
	...
	*/
	...
	PieceManager.prototype.resetPanels = function(){
		for (k in this.panelList) {
			this.panelList[k].resetPanel();
		}
	}
	
	/**
	*Sort pieces in the panel
	*@param 	* 
	*/

	PieceManager.prototype.sortPieces = function(){
		for (k in this.panelList) {
			this.panelList[k].sortPieces();
			}
	}
	...
	*/
	PieceManager.prototype.addPanel = function(cp,temp){
		...
		panel.colLen = parseInt(panel.parent.getBounds().height/(125*panel.parent.scaleY))-2;
		if (!temp){
	...
	*/
	PieceManager.prototype.setConclusion =  function(st_list){
		...
		var piece_num_text = new createjs.Text(cp.pieceNum, "50px Arial","red");
		cp.x = cp.homeX = this.nPosX;
		cp.y = cp.homeY = this.nPosY;
		this.nextPiecePosition();
		this.getMatchingPiecesPositions(cp);
	...
	*/
	PieceManager.prototype.pieceReceived = function(st_list){
		...
		else if(!this.checkPiece(st_list)){
